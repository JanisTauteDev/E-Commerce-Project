/* NORDHEM - shop page: filters, sort, search, tier filtering. */
(function () {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const state = {
      categories: new Set(params.get('category') ? [Number(params.get('category'))] : []),
      tiers: new Set(),
    ...
    function apply() {
      return PRODUCTS
        ...
        .filter(p => state.tiers.size === 0 || p.variants.some(v => state.tiers.has(v.tier)))
        .filter(p => !state.q || p.name.toLowerCase().includes(state.q) || p.description.toLowerCase().includes(state.q))
