...
const _photos = {
  1:'fjord-sofa.webp',     2:'lagom-coffee-table.webp', 3:'bjork-bookshelf.webp',
  4:'drom-bed-frame.webp', 5:'natt-nightstand.webp',    6:'linne-wardrobe.webp',
  7:'koka-cookware-set.webp', 8:'bord-dining-table.webp', 9:'glas-tumbler-set.webp',
  10:'skriva-desk.webp',   11:'sitta-office-chair.webp', 12:'ordna-organiser.webp',
  13:'bada-towel-set.webp', 14:'spegel-mirror.webp',    15:'tval-soap-dispenser.webp',
  16:'mane-floor-lamp.webp', 17:'stjarna-pendant.webp', 18:'ljus-table-lamp.webp',
  19:'mys-armchair.webp',  20:'kniv-knife-block.webp',
};

/* Parse the rows of a single `INSERT INTO <table> (cols) VALUES (...),(...);` statement */
function _parseInsert(sql, table) {
  const re = new RegExp('INSERT\\s+INTO\\s+' + table + '\\s*\\(([^)]*)\\)\\s*VALUES\\s*([\\s\\S]*?);', 'i');
  const m = re.exec(sql);
  if (!m) return [];
...
async function _loadFromDatabase() {
  ...
  if (!res.ok) throw new Error('Could not load database/schema.sql (HTTP ' + res.status + ')');
  ...
  _parseInsert(sql, 'product_variants').forEach(r => {
    (variantsByProduct[r.product_id] = variantsByProduct[r.product_id] || []).push({
      tier: r.tier,
      sku: r.sku,
      price: r.price,
      stock: r.stock,
  ...
  const products = _parseInsert(sql, 'products').map(r => ({
    ...
    variants: (variantsByProduct[r.id] || []).sort((a, b) => order[a.tier] - order[b.tier]),
  }));
...
NORDHEM.ready = (async function () {
  try {
    ...
    console.error('[NORDHEM] Failed to load product data from the database.', err);
    console.error('[NORDHEM] Serve the site over HTTP (e.g. a local web server) so database/schema.sql can be fetched.');
  }
